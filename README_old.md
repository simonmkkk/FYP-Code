...
- **Python 版本**：Python 3.13.7（已測試）或 Python 3.8+
- **作業系統**：Windows / macOS / Linux
- **記憶體**：建議至少 4GB RAM（進行大規模模擬時）
- **處理器**：支援多核心處理器以加速並行計算
- **套件管理器**：uv（推薦）或 pip
...
| 套件 | 版本 | 用途 |
|------|------|------|
| numpy | 2.3.3 | 數值計算和陣列操作 |
| matplotlib | 3.10.6 | 繪圖和可視化 |
| joblib | 1.5.2 | CPU 並行計算支援 |
| tqdm | 4.67.1 | 進度條顯示 |
...
| 套件 | 版本 | 用途 |
|------|------|------|
| pandas | 2.3.3 | 數據分析和處理 |
| scipy | 1.16.2 | 科學計算函數庫 |
...
```bash
python -c "import numpy, matplotlib, joblib, tqdm, pandas, scipy; print('✅ 環境配置成功！')"
```
...
│
├── 📁 data_csv/                         # 💾【CSV匯出層】數據匯出
│   ├── __init__.py
│   └── file_io.py                       # 【CSV匯出】模擬結果匯出
│
├── 📁 scripts/                          # 📜【腳本層】獨立執行腳本
...
圖表保存位置：`data_graph/graphs/paper_reproductions/`
...
3. 檢查 `data_graph/results/latest/` 中的示例輸出
