...
def formula_5_collision_raos_approx(device_count, N):
    """
    論文公式(5): 碰撞RAO近似公式
    N_{C,i} ≈ N * (1 - exp(-K_i/N) * (1 + K_i/N))
    
    Args:
        device_count: 競爭設備數量 (可以是M或K_i)
        N: RAO總數
    
    Returns:
        float: 碰撞RAO的近似數量
    """
    exp_term = np.exp(-device_count/N)
    N_C = N * (1 - exp_term * (1 + device_count/N))
    return N_C

# 保持向後兼容的別名函數
def formula_4_success_raos_approx(device_count, N):
    """向後兼容: 調用論文公式(4)"""
    return paper_formula_4_success_approx(device_count, N)

def formula_5_collision_raos_approx(device_count, N):
    """向後兼容: 調用論文公式(5) - 但使用更準確的實現"""
    exp_term = np.exp(-device_count/N)
    N_C = N * (1 - exp_term * (1 + device_count/N))
    return N_C

def formula_6_success_devices_per_ac(K_i, N):
    """向後兼容: 調用論文公式(6)"""
    return paper_formula_6_success_per_cycle(K_i, N)

def formula_7_remaining_devices_correct(K_i, N_i):
    """
    論文公式(7): 下一個AC的競爭設備數 - 正確實現
...
def formula_7_remaining_devices(K_i, N_s_i):
    """
    論文公式(7): 下一個AC的競爭設備數 - 簡化版本
    K_{i+1} = K_i - N_{S,i}
    ...
    Returns:
        float: 下一個AC的競爭設備數
    """
    return max(0, K_i - N_s_i)

def formula_8_access_success_probability(N_s_list, M):
    """
    論文公式(8): 接入成功概率
    P_S = Σ(i=1 to I_max) N_{S,i} / M
...
def formula_9_average_access_delay(N_s_list):
    """
    論文公式(9): 平均接入延遲
    T_a = Σ(i=1 to I_max) i * N_{S,i} / Σ(i=1 to I_max) N_{S,i}
...
def formula_10_collision_probability(N_c_list, I_max, N):
    """
    論文公式(10): 碰撞概率
    P_C = Σ(i=1 to I_max) N_{C,i} / Σ(i=1 to I_max) N_i
